        ...
        try:
            ...
            if json_string:
                ...
                try:
                    ...
                    log_func(f"Initial JSON parsing failed (response_mime_type='application/json'): {e}. Raw response:\n{json_string[:1000]}", "error")
...
def call_gemini_text_analysis(text_content, api_key, model_name, prompt, log_func,
        ...
        try:
            ...
            if block_reason:
                ...
                if all_blocked:
                    ...
                    log_func(error_msg, level="error"); continue
                else:
            ...
            if raw_response_text:
                try:
                 ...
                 if not candidates_exist or finish_reason_val != finish_reason_stop:
                      feedback = "N/A"
                      try: # Line 367 area in the latest code
                          feedback = str(response.prompt_feedback) if response.prompt_feedback else "N/A"
                      except Exception:
                          pass # CORRECTED: Moved to new line
    ...
    if not all_parsed_data:
        log_func("Warning: No data was successfully extracted from any text chunk.", "warning"); return []
    ...
    else: log_func("Error saving final combined results.", "error")
...
def tag_tsv_rows_gemini(data_rows_with_header, api_key, model_name, system_prompt,
    ...
    """
    if not data_rows_with_header: log_func("No data rows provided for tagging.", "warning"); return
    ...
    if not configure_gemini(api_key):
        ...
        for row in data_rows: yield row + ["ERROR: API Key Config Failed"]; return
    ...
    for i in range(0, total_rows, batch_size):
        ...
        for row_index, row in enumerate(batch):
            ...
            batch_tagged_rows.append(output_row_list)

        all_tagged_rows.extend(batch_tagged_rows)
...
def save_tsv_incrementally(rows_list, output_dir, base_filename, step_name, log_func):
    ...
    try:
        ...
        log_func(f"Saved intermediate {step_name} results ({len(rows_list)-1} data rows) to {temp_filename}", "debug"); return temp_filepath
